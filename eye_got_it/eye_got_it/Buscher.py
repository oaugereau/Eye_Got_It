...
def fixationBuscher2008(gazes,usFixation=80000,smallSquareSize=30,bigSquareSize=50,consecutiveFails=4):
    ...
    while i < len(gazes):
        ...
        if insideSquare(fixCandidate,smallSquareSize):
            ...
            for gaze in fixCandidate:
                gazesInFixations.append(gaze)
        else:
            i+=1
    #making the % of gazes which are in a fixation
    percentageGazes = 100*(len(gazesInFixations)/len(gazes))
    #print("POURCENTAGE DE GAZES DANS FIXATION", 100*(len(gazesInFixations)/len(gazes)))
    return fixations, percentageGazes
